...
export function useTrackedTokenPairs(): [Token, Token][] {
  ...
  // pairs for every token against every base  每个标记都与每个pairs配对
  const generatedPairs: [Token, Token][] = useMemo(
    () =>
      chainId
        ? flatMap(Object.keys(tokens), (tokenAddress) => {
            const token = tokens[tokenAddress]
            // for each token on the current chain,对于当前链上的每个令牌，
            return (
              // loop though all bases on the current chain 遍历当前链上的所有基础
              (BASES_TO_TRACK_LIQUIDITY_FOR[chainId] ?? [])
                // to construct pairs of the given token with each base  用每个pairs构造的给定令牌对
                .map((base) => {
